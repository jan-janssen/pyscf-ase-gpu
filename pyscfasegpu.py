...
class PySCF(Calculator):
	...
	def get_potential_energy(self, atoms=None, force_consistent=False):
		mf = rks.RKS(gto.M(atom=ase_atoms_to_pyscf(atoms), basis=self.basis, charge=0, verbose=0))
		mf.xc = self.xc
		energy = mf.kernel()
	...
	def get_forces(self, atoms=None):
		mf = rks.RKS(gto.M(atom=ase_atoms_to_pyscf(atoms), basis=self.basis, charge=0, verbose=0)).run()
		mf.xc = self.xc
